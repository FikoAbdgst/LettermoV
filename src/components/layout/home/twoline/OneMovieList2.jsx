...
const OneMovieList2 = ({
    ...
    selectedCategory,
    currentSlideMovie2,
    setCurrentSlideMovie2,
    moviesPerSlide2,
    selectedType,
    sortOption,
    selectedSortYearOption,
    selectedSortAlphabetOption,
    minRating
}) => {
    ...
    useEffect(() => {
        const handleResize = () => {
            if (window.innerWidth < 640) { // Mobile
                setGridCols("grid-cols-3"); // 3 films per row on mobile
    ...
    const movieMatchesCategories = (movie) => {
        if (selectedCategory.length === 0) {
            return true;
        }

        return selectedCategory.every(category => {
            const selectedGenreId = allGenres?.find(genre => genre.name === category)?.id;
            if (selectedGenreId === undefined) return false;
            return movie.genre_ids.includes(selectedGenreId);
        });
    };

    const movieMatchesYear = (movie) => {
        const releaseDate = selectedType === 'Movies' ? movie.release_date : movie.first_air_date;
        const sortYearOption = selectedSortYearOption !== 'Year' ? selectedSortYearOption.toString() : '';
        const releaseDateString = releaseDate ? releaseDate.split('-')[0] : '';
        return sortYearOption === '' || releaseDateString.startsWith(sortYearOption);
    };

    const movieMatchesAlphabet = (movie) => {
        if (selectedSortAlphabetOption === 'Alphabet') return true;
        const title = movie.title || movie.name || '';
        const firstLetter = title[0]?.toUpperCase() || '';
        return firstLetter === selectedSortAlphabetOption;
    };

    const movieMatchesRating = (movie) => {
        if (minRating === 0) return true;
        const ratingRangeStart = Math.floor(parseFloat(minRating));
        const ratingRangeEnd = ratingRangeStart + 0.9;
        const movieRating = parseFloat(movie.vote_average.toFixed(1));
        return movieRating >= ratingRangeStart && movieRating <= ratingRangeEnd;
    };

    const sortMovies = (movies) => {
        if (sortOption === 'Newest') {
            return movies.sort((a, b) => {
                const dateA = selectedType === 'Movies' ? new Date(a.release_date) : new Date(a.first_air_date);
                const dateB = selectedType === 'Movies' ? new Date(b.release_date) : new Date(b.first_air_date);
                return dateB - dateA;
            });
    ...
    const filteredMovies = movies
        .filter(movie => movieMatchesCategories(movie) &&
            movieMatchesYear(movie) &&
            movieMatchesAlphabet(movie) &&
            movieMatchesRating(movie));
    ...
    const handleSaveMovieId = (id, type) => {
        localStorage.setItem('selectedMovieId', id);
        if (type === "movies") {
            localStorage.setItem('selectedMediaType', 'movie');
        } else if (type === "series") {
            localStorage.setItem('selectedMediaType', 'tv');
        }
    };

    if (limitedMovies.length === 0) {
        return (
            <div className="w-full h-48 md:h-96 flex flex-col justify-center items-center">
                <img src="../../../../../public/images/pop.png" alt="pop" className="w-20 h-20 md:w-32 md:h-32 lg:w-40 lg:h-40" />
                <p className="mt-2 text-sm md:text-lg font-semibold">There is no movie🥲</p>
                <p className='text-xs md:text-sm font-bold text-gray-600'>Look again so you won't be disappointed 😊</p>
            </div>
        );
    }
                            ...
                            <div className="mb-1 sm:mb-2 relative group overflow-hidden rounded-lg">
                                ...
                                <Link to={'/detail'}>
                                    <button
                                        ...
                                        onClick={() => handleSaveMovieId(movie.id, selectedType.toLowerCase())}
